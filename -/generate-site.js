...
if (customerData.reservation_url && customerData.reservation_text) {
  reservationButton = `<div class="reservation-button-container">
          <a class="reservation-btn" href="${customerData.reservation_url}" target="_blank" rel="noopener">
            <span class="reservation-text">${customerData.reservation_text}</span>
          </a>
        </div>`;
}
finalHtml = finalHtml.replace('{{ reservation_button }}', reservationButton);
...
// その他のプレースホルダーを置換
const excludedKeys = ['gallery_images', 'sns_links', 'sns', 'reservation_url', 'reservation_text', ...Object.keys(snsConfig).map(k => snsConfig[k].url)];
